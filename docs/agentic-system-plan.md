...
| **1a · Retrieval (offline)** (~1.5d) | hybrid resolver (dense + BM25 + fuzzy, RRF) + `build-index` + `eval/run_retrieval.py` + the ablation table. **No agent, no LLM.** See [retrieval-pipeline-plan.md](retrieval-pipeline-plan.md) | event-resolution **P/R ≥ 0.85** on the taxonomy, standalone; hybrid provably > dense-only |
| **1b · Foundation (+A)** (~1.5d) | the **shared foundation** — the 5 tools (retrieval from 1a) + analytics compiler + guardrails — plus System A as its first consumer, wired to `eval/run_case`. (A may be owned by Shubham; it imports this foundation.) | A runs on `inst_001` end-to-end → a (bad) `Hypothesis` → scored by the harness → fully traced. *The whole loop proven on the baseline.* |
